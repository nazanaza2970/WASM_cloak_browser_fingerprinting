...
def process_all_trace_folders(base_input_dir="files/traces", base_output_dir="files/features"):
    """
    Wrapper function to run process_all_traces() on each folder under files/traces/.
    Each output file will be saved as files/features/<folder_name>.csv.
    """
    # Ensure output directory exists
    os.makedirs(base_output_dir, exist_ok=True)

    # Iterate over all subdirectories in base_input_dir
    for folder_name in os.listdir(base_input_dir):
        ...
        # Input pattern for JSON files
        # input_pattern = os.path.join(folder_path, "*.json")
        input_pattern = folder_path

        # Output file path
        output_file = os.path.join(base_output_dir, f"{folder_name}.csv")

        print(f"Processing folder: {folder_path}")
        print(f" → Output: {output_file}")

        try:
            process_all_traces(input_pattern, output_file)
        except Exception as e:
            print(f"Error processing {folder_name}: {e}")


def process_all_traces(input_path, output_path):
    """
    Process multiple API log files, extract features, and save to an Excel file (one row per script).
    """
    all_files = glob.glob(f"{input_path}/*json")
    all_rows = []

    for file_path in all_files:
        script_number = file_path.split("/")[-1].split(".")[0][:-5].split("_")[-1]
        # print(script_number)
        try:
        ...
        for script_hash, script_data in data.items():
            ...
            # Structure raw features
            raw_features = {
                "content_hash": script_number,
                "ground_truth": ground_truth,
            }

            # Merge in the flattened features
            raw_features.update({f"onegram_{k}": v for k, v in one_grams.items()})
            raw_features.update({f"valfeat_{k}": v for k, v in custom_features_val.items()})
            raw_features.update({f"argfeat_{k}": v for k, v in custom_features_arg.items()})

            # Apply one-hot encoding (if needed)
            one_hot_encoded = one_hot_encode_features(raw_features, FEATURE_TYPE_MAP)
            raw_features.update(one_hot_encoded)

            all_rows.append(raw_features)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--input', required=True, help="Path to the input API log file or pattern for multiple files.")
    # parser.add_argument('--output', required=False, help="Path to the output JSON file for processed features.")
    # args = parser.parse_args()
    # if args.output:
    #     process_all_traces(args.input, args.output)
    # else:
    #     process_traces(args.input)