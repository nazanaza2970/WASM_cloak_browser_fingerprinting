...
from html_generator_conversion import html_generator_conversion


# In[4]:

html_file_path = "/private_persistent_data/generated_webpage_for_test_code.html"
local_url = "0.0.0.0:8000/"+html_file_path
...
# In[22]:
async def test_website_in_browser(url):
    errors_list = []
    ...
    async with async_playwright() as p:
        try:
            ...
            await page.goto(url, wait_until="load", timeout=3000)
            await page.wait_for_timeout(500) 
            await browser.close()
            
        except Error as e:
            errors_list.append(f"Playwright navigation error: {e}")
        except Exception as e:
            errors_list.append(f"An unexpected error occurred: {e}")

    return {"valid": len(errors_list) == 0, "errors": errors_list}

# async def set_html_page(code,path):
#     code_page = html_generator_conversion(code)
#     try:
...
async def validate_js_code(code,html_file_path):
    ...
    try:
        ...
        return {"valid": False, "errors": [f"Page saving failed"]}
    browser_test_result = await test_website_in_browser(local_url)
...
async def single_rule_check(js_code,rule_fn, ast_g):
    ...
    validation_result = await validate_js_code(converted_code,html_file_path)
...
async def main():
    ...
    samples_to_convert = glob.glob('/samples_for_conversion/*')
    
    #[1:4]
    start_from_set_index=True
    for sample in tqdm(samples_to_convert[0:1]):
        ...
        buffer = []
        # csv_file = f''
        csv_file = f'/private_persistent_data/conversion/conversion_logs/conversion_log_{base_name}.csv'
        write_header=True
        ...
        if start_from_set_index:
            start_from_set_index=False
        else:
            start_index=0
        count = start_index
        ...
        for i in tqdm(range(start_index, len(data)), initial=start_index, total=len(data)):
            ...
            if working_rules and time_fail==0:
                try:
                    ...
                    v = await validate_js_code(converted_code,html_file_path)
            ...
            if count % 50 == 0:
                ...
                json_count=count//50
                with open(f'/conversion_output_new/js_{base_name}_{json_count}.json','w') as f:
                # with open(f'_{json_count}.json','w') as f:
                    json.dump(js,f)
                with open(f'/conversion_output_new/wasm_{base_name}_{json_count}.json','w') as f:
                # with open(f'_{json_count}.json','w') as f:
                    json.dump(wasm,f)
                js={}
                wasm={}
