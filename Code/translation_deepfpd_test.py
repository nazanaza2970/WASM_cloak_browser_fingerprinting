...
def replace_literals_recursive(js_code, ast_g, node=None, chunks_to_merge=None,asm_code=None):
    ...
    if asm_code is None:
        asm_code = []
    if node is None:
        # Parse the root node if no node is provided
        node = ast_g
        # node = esprima.parseScript(js_code, loc=True)

    def process_declaration(declarator, start_pos, end_pos):
        # nonlocal assemblyscript_code
        ...
        if declarator.init and declarator.init.type == "Literal":
            ...
            if isinstance(declarator.init.value, str):
                ...
                # wasm_bytes = compile_asm(assemblyscript_code)
                binding_code = get_binding_code_string_literal(var_name,start_ind)  # need new code
                ...
                # wasm_bytes = compile_asm(assemblyscript_code)
                binding_code = get_binding_code_general(var_name,start_ind)  # need new code
    ...
    if hasattr(node, 'body') and isinstance(node.body, list):
        for child in node.body:
            replace_literals_recursive(js_code, child, chunks_to_merge,asm_code)
        ...
        # Skip literals in the initialization, condition, and iteration expressions of the for loop
        for part in ['init', 'test', 'update']:
            if hasattr(node, part) and getattr(node, part) and getattr(node, part).type == "Literal":
                # Avoid processing literals in any part of the for loop
                continue
        # Recursively process the body of the for loop
        replace_literals_recursive(js_code, node.body, chunks_to_merge,asm_code)
    elif hasattr(node, 'expression') and node.expression:
        replace_literals_recursive(js_code, node.expression, chunks_to_merge,asm_code)
    elif hasattr(node, '__dict__'):
        for key, value in node.__dict__.items():
            if isinstance(value, list):
                for item in value:
                    if isinstance(item, dict) or hasattr(item, '__dict__'):
                        replace_literals_recursive(js_code, item, chunks_to_merge,asm_code)
            elif isinstance(value, dict) or hasattr(value, '__dict__'):
                replace_literals_recursive(js_code, value, chunks_to_merge,asm_code)
...
async def main():
    ...
    js={}
    wasm={}
    with open('script_with_content.json','r') as f:
        data = json.load(f)

    buffer = []
    csv_file = f'/persistent_data/deepFPD_test_conversion.csv'
    write_header=True
    ...
    for i in tqdm(range(start_index, len(data)), initial=start_index, total=len(data)):
        ...
        if count % 50 == 0:
            ...
            json_count=count//50
            with open(f'/conversion_output/DeepFPD/js_{json_count}.json','w') as f:
                json.dump(js,f)
            with open(f'/conversion_output/DeepFPD/wasm_{json_count}.json','w') as f:
                json.dump(wasm,f)
            js={}
            wasm={}
            # flush_buffer(buffer, csv_file, write_header)
